...
3a. cargo install --force cargo-make
4. cd ~
...
main build commands:
1. cd wasm && ./build.sh --release && cd -
2. cd ts && npm install && npm run build:prod && cd -
3. cd cli && node build_html.js && cd -
...
27. ios (even on chrome) only through workers (about opfs) 
